...
def load_fonts(font_or_dir):
    ...
    if not p.exists(): # 如果 p 不存在，调用 sys.exit() 函数终止程序，打印错误信息([Error]...)
        sys.exit(f"[Error] Font path does not exist: {font_or_dir}")

    if p.is_file():
        # 返回一个包含单个元素的列表，这个元素是路径 p 的字符串形式，即具体的字体文件路径
        return [str(p)]
    else:
        #  利用列表推导式遍历路径 p 下所有条目；检查它是否是一个文件 if font.is_file()。如果是，则将其转换为字符串并添加到 font_paths 列表中。
        font_paths = [str(font) for font in p.glob('*') if font.is_file()] #
        if not font_paths:
            sys.exit(f"[Error] No font files found in directory: {font_or_dir}")
        # 返回包含这些文件路径的 font_paths 列表。
        return font_paths
...
def main():
    ...
    # mkdir()创建 output_dir 指定的目录，如果父目录不存在，parents=True指示创建所有必要的父目录；exist_ok=True存在则静默处理
